...
import fr.decade.pfe.storefront.updates.FormUpdateProfile;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	public String editProfile(final Model model) throws CMSItemNotFoundException
	{
		...
		final CustomerData customerData = customerFacade.getCurrentCustomer();
		final FormUpdateProfile updateProfileForm = new FormUpdateProfile();
		...
		{
			final String postalCode = billingAddress.getPostalCode();
			if (postalCode != null)
			{
				updateProfileForm.setPostalCode(postalCode);
			}
		}
	...
	@RequireHardLogIn
	public String updateProfile(final FormUpdateProfile updateProfileForm, final BindingResult bindingResult, final Model model,
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (StringUtils.isNotBlank(updateProfileForm.getPostalCode()))

		{
			final AddressData billingAddress = new AddressData();
			billingAddress.setPostalCode(updateProfileForm.getPostalCode());
			customerData.setDefaultBillingAddress(billingAddress);
		}
