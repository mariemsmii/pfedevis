 ...
 */
package fr.decade.pfe.storefront.controllers.pages;
...
import fr.decade.pfe.storefront.controllers.ControllerConstants;
import fr.decade.pfe.storefront.updates.PfeUpdateState;
...
public class MyQuotesController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_QUOTE_URL = REDIRECT_PREFIX + "/my-account/my-quotes/{quoteCode}";
	...
	@Resource(name = "pfeCreateOrderFromQuoteFacade")
	private PfeCreateOrderFromQuoteFacade pfeCreateOrderFromQuoteFacade;
	...
	final String sortCode, final Model model) throws CMSItemNotFoundException
	{
		...
		final List rslt = searchPageData.getResults();
		return getViewForPage(model);
	}

	@RequestMapping(method = RequestMethod.POST)
	@RequireHardLogIn
	public String quotesUpdateState(@ModelAttribute
	final PfeUpdateState pfeUpdateState, @RequestParam(value = "quoteCode")
	final String quoteCode, final Model model, final HttpServletRequest request, @RequestParam(value = "action", required = true)
	final String action) throws CMSItemNotFoundException
	{
		...
		quoteData.setState(pfeUpdateState.getState());
		LOG.info(request.getParameter(quoteCode));

		if (request.getParameter("action") != null)
		{
			if (action.equals("VALID"))
			{
				getPfeDefaultQuoteFacade().updateStateToBuyerApproved(quoteCode);
				getPfeCreateOrderFromQuoteFacade().createOrderFromQuote(quoteCode);
			}
			else if (action.equals("CANCEL"))
			{

				quoteData.setState(pfeUpdateState.getState());
				getPfeDefaultQuoteFacade().updateStateToBuyerReject(quoteCode);
			}
		}
		model.addAttribute("pfeUpdateState", pfeUpdateState);
	...
	final String quoteCode) throws CMSItemNotFoundException
	{
		try
		{
			...
			final PfeUpdateState pfeUpdateState = new PfeUpdateState();
	...
	@RequestMapping(value = "/{quoteCode}", method = RequestMethod.POST)
	@RequireHardLogIn
	public String quoteUpdateState(@PathVariable("quoteCode")
	final String quoteCode, @ModelAttribute
	final PfeUpdateState pfeUpdateState, final Model model, final HttpServletRequest request,
	...
	/**
	 * @return the pfeDefaultQuoteFacade
	 */
	public PfeDefaultQuoteFacade getPfeDefaultQuoteFacade()
	{
		return pfeDefaultQuoteFacade;
	}
	...
	/**
	 * @return the pfeCreateOrderFromQuoteFacade
	 */
	public PfeCreateOrderFromQuoteFacade getPfeCreateOrderFromQuoteFacade()
	{
		return pfeCreateOrderFromQuoteFacade;
	}

	/**
	 * @param pfeCreateOrderFromQuoteFacade
	 *           the pfeCreateOrderFromQuoteFacade to set
	 */
	public void setPfeCreateOrderFromQuoteFacade(final PfeCreateOrderFromQuoteFacade pfeCreateOrderFromQuoteFacade)
	{
		this.pfeCreateOrderFromQuoteFacade = pfeCreateOrderFromQuoteFacade;
	}
